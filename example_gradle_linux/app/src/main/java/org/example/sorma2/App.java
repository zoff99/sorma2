...
public class App {
    ...
    private static final String TAG = "Sorma2-Example-Gradle:";
    ...
    static void upgrade_db_schema_do(int old_version, int new_version)
    {
        if (new_version == 1) {
    ...
    private static OrmaDatabase OrmaDatabase_wrapper(String dbs_path, String pref__db_secrect_key, boolean pref__db_wal_mode)
    {
        set_schema_upgrade_callback(new OrmaDatabase.schema_upgrade_callback()
        {
            ...
            {
                System.out.println(TAG + "trying to upgrade schema from " + old_version + " to " + new_version);
    ...
    private static void startSelectAllWithOrma()
    {
    ...
    private static void startInsertWithOrma()
    {
        long start = System.currentTimeMillis();
        for (int j = 0; j < N_OPS; j++)
        {
    ...
    public static void main(String[] args) {
        ...
        String dbs_path = path + "/" + MAIN_DB_NAME;
